...
ini_set('display_errors', 0);
	...
	if($_POST['mode']=='send') {
		...
		if($_SESSION['last_token'] != $_POST['_token']) {
			// 送信処理
			$support_addr = 'saiyo@naniwa-co.jp';
...
<head>
	...
	<title>採用お問合わせ | 株式会社ナニワ</title>
	<meta name="keywords" content="株式会社ナニワ" />
	<meta name="description" content="新卒・中途採用の問い合わせなど、必要事項にご入力の上ご送信下さい。" />
...
.agree_check+div > label {
	display: block;
	margin: 20px auto 40px;
}
.agree_check+div input+label {
	color: #da003c;
	font-size: 85%;
	position: absolute;
	margin-top: 1em;
	...
	display: block;
			...
			<div class="agree_check">
				<iframe src="ssi/privacy.html" frameborder="0" width="100%"></iframe>
			</div>
			<div align="center">
				<label><input type="checkbox" name="agree" value="1" />同意する</label>
			</div>
